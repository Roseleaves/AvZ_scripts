...
void Script()
{
    ...
    // Card(LILY_PAD, 3, 6);
    plant_fixer.start(PUMPKIN, {{3, 7}, {3, 9}}, 40);

    // SetTime(-598, 1),
    // Card(M_ICE_SHROOM, 3, 9);
    // SetTime(-596, 1),
    ice_filler.start({{4, 9}, {3, 1}, {3, 9}, {2, 9}});
    ...
    SetTime(ICE_TIME - COFFEE_ICE, 1),
        ice_filler.coffee(),
        ice_filler.resetFillList({{4, 9}, {3, 1}, {3, 9}});
    // /*
    // IS-PS, PDc
    // PDc
    ...
    InsertOperation([=]() {
        ...
        for (int index = 0; index < GetMainObject()->zombieTotal() && !flag1; ++index) {
            // 如果在 1 路有红眼僵尸
        ...
        if (flag1 || flag2) { //一路或二路有红眼，不用垫下半场
            SetTime(1800, 9),
    ...
    Delay(template01), pao_operator.pao(2, 9);
    Delay(template01), pao_operator.pao(5, 9);
    Delay(template01), pao_operator.pao(2, 9);
    Delay(template01), pao_operator.pao(5, 9);
    Delay(template01), pao_operator.pao(2, 9);
    Delay(template01), pao_operator.pao(5, 9);
    Delay(template01), pao_operator.pao(3, 5);
    Delay(template01 + 160), pao_operator.pao(3, 2);
}